...
public class SuperWiFi2 extends LocalizeActivity {
    /*****************************************************************************************************************
    ...
    private String FileNameGroup[]={"xie","CMCC-nGKE","@PHICOMM_87","ChinaNet-5cDf",
            "liaozeming","yxb","ylylzx","HUAWEI-QJZ7AF","wuguangming"};
    ...
    private Vector<Integer> scanned = null;
    boolean isScanning = false;

    public SuperWiFi2(Context context)
    {
        this.mWiFiManager = (WifiManager)context.getSystemService(Context.WIFI_SERVICE);
        this.scanned = new Vector<Integer>();
    }
    ...
    public void scanForLoc()
    {
        this.isScanning = true;
        Thread scanAndPosThread = new Thread(new Runnable() {
            public void run() {
                scanned.clear();
                //write2file(FileLabelName + ".txt", "ID" + recordID+ "\r\n");

                //String tmpstr1=Double.toString(x);
                //String tmpstr2=Double.toString(y);
                //write2file(FileLabelName + ".txt", posx + "\r\n");
               ...
               // write2file(FileLabelName + ".txt", posy+ "\r\n");
                ...
                int CurTestTime = 1;
                while(CurTestTime++ <= TestTime) performScanMod();
                for (int index = 1;index <= NumberOfWiFi; index++)
                {//Record the average of the result
                    scanned.add(RSS_Value_Record[index - 1] / RSS_Measurement_Number_Record[index - 1]);
                    //write2file(FileLabelName + ".txt",RSS_Value_Record[index - 1] / RSS_Measurement_Number_Record[index - 1] + "\r\n");
                }
                isScanning=false;
            }
        });
        scanAndPosThread .start();
    }


    private void performScanMod()
    {
    ...
    public Vector<Integer> getRSSlist(){
        return scanned;
    }
